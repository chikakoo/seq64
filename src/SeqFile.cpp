...
MidiFile* SeqFile::toMIDIFile(ROM& rom) {
    ...
    while (a < data.size()) {
        ...
        if (action == "Unknown") {
            ...
            param = command.getChildWithProperty(idMeaning, "Value");
            if (!param.isValid()) {
                SEQ64::say("Chn Reverb event with no value!");
                continue;
            }
            value = getAdjustedValue(param);
            ...
            param = command.getChildWithProperty(idMeaning, "Value");
            if (!param.isValid()) {
                SEQ64::say("Chn Vibrato Depth event with no value!");
                continue;
            }
            value = getAdjustedValue(param);
            ...
            param = command.getChildWithProperty(idMeaning, "Value");
            if (!param.isValid()) {
                SEQ64::say("Chn Vibrato Delay event with no value!");
                continue;
            }
            value = getAdjustedValue(param);
            msg = MidiMessage::controllerEvent(channel + 1, 78, value);
            msg.setTimeStamp(t * ticks_multiplier);
            mtracks[channel]->addEvent(msg);
        }
        else if (action == "Chn Release Rate") {
